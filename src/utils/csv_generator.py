...
def get_files(folders, data_root="", descriptor="", filetype="tif"):
    ...
    for folder in folders:
        files = glob.glob(
            os.path.join(data_root, folder, "*" + descriptor + "*." + filetype)
        )
        filelist.extend(
            [os.path.join(folder, os.path.split(f)[-1]) for f in files]
        )
...
def create_apply_csv(data_list, save_path, save_name):
    create_csv(
