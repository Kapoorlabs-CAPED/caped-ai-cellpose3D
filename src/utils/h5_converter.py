...
def h5_writer(data_list, save_path, group_root="data", group_names=["image"]):
    ...
    assert len(data_list) == len(
        group_names
    ), "Each data matrix needs a group name"
    ...
    with h5py.File(save_path, "w") as f_handle:
        ...
        for data, group_name in zip(data_list, group_names):
            grp.create_dataset(
                group_name, data=data, chunks=True, compression="gzip"
            )
...
def prepare_images(
    ...
    for image_folder in folders:
        image_list = glob.glob(
            os.path.join(data_path, image_folder, identifier)
        )
        for num_file, file in enumerate(image_list):
            print_timestamp(
                'Processing image {0}/{1} in "{2}"',
                (num_file + 1, len(image_list), image_folder),
            )
            ...
            save_name = os.path.join(
                data_path, image_folder, descriptor + save_name[:-4] + ".h5"
            )
            h5_writer(
                save_imgs,
                save_name,
...
def prepare_masks(
    ...
    for mask_folder in folders:
        ...
        experiment_identifier = (
            "corrupt" + str(corrupt_prob).replace(".", "")
            if corrupt_prob > 0
            else ""
        )
        os.makedirs(
        ...
        for num_file, file in enumerate(mask_list):
            print_timestamp(
                f"Processing mask {0}/{1} in {2}",
                (num_file + 1, len(mask_list), mask_folder),
            )
